...
import FormPaketReguler from "../../Form/FormPaketReguler";

const PaketRegulerTersedia = () => {
    const [paketReguler, setPaketReguler] = useState([]);
    const [isModalOpen, setIsModalOpen] = useState(false);
    const [selectedReguler, setSelectedReguler] = useState(null);
    ...
    const handleDelete = async (id) => {
        if (window.confirm("Apakah Anda yakin ingin menghapus data ini?")) {
            try {
                const response = await axios.delete(`http://localhost:5000/pkt_reguler/delete/${id}`);
                    ...
                    <div className="container mx-auto">
                        <div className="bg-white shadow-md rounded-lg">
                            <div className="card-body flex justify-between items-center">
                                <div>
                                    <h2 className="text-3xl lg:text-4xl mb-5 p-2 mt-2 font-bold ml-3">Paket Reguler</h2>
...
export default PaketRegulerTersedia;
